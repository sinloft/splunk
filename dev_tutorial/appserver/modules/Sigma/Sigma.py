...
import logging
logger = logging.getLogger('splunk.module.Sigma')

import math
import cgi

class Sigma(module.ModuleHandler):
    ...
    def generateResults(self, host_app, client_app, sid, count=1000, 
        ...
        if not sid:
            raise Exception('Sigma.generateResults - sid not passed!')

        try:
            job = splunk.search.getJob(sid)
        except splunk.ResourceNotFound, e:
            logger.error('Sigma could not find job %s. Exception: %s' % (sid, e))
