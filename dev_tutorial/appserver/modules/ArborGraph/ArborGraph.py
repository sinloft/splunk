...
import logging
logger = logging.getLogger('splunk.module.ArborGraph')

import math
import cgi

class ArborGraph(module.ModuleHandler):
    ...
    def generateResults(self, host_app, client_app, sid, count=1000, 
        ...
        if not sid:
            raise Exception('ArborGraph.generateResults - sid not passed!')

        try:
            job = splunk.search.getJob(sid)
        except splunk.ResourceNotFound, e:
            logger.error('ArborGraph could not find job %s. Exception: %s' % (sid, e))
